...
const rel = env.LMNC_BINARY ? null : PLATFORMS?.[platform]?.[arch];
let bin = env.LMNC_BINARY || null;
...
if (bin && !existsSync(bin)) {
  console.error(`LMNC_BINARY is set to ${bin}, and no file is there.`);
  process.exitCode = 1;
} else if (unresolved) {
  console.error(
    `lmnc supports ${platform} ${arch}, and its binary package is not ` +
  ...
  console.error(
    `lmnc ships no prebuilt binary for ${platform} ${arch}. ` +
      "Install it with `cargo install lmnc`, or set LMNC_BINARY to a path."
  );
  process.exitCode = 1;
} else {
  ...
  if (result.error) {
    ...
    console.error(`lmnc cannot run ${bin}: ${result.error.message}`);
    process.exitCode = 1;
    ...
    console.error(`lmnc: ${bin} was killed by ${result.signal}`);
