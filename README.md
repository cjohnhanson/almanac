...
`almanac` is taken on crates.io, PyPI and npm alike, so every registry
carries `lmnc`. The tap carries `almanac`. The install puts both names on
your path. Type `almanac`.
...
page](https://github.com/cjohnhanson/almanac/releases) holds all of them.
