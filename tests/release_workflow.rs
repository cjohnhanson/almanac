...
//! archive under a name the build never uploads, with every suite
//! green. These tests read the workflow against the files it names, so
//! the next rename fails here and not on a tag.
...
const SHORT: &str = "lmnc";
/// The binary the build job archives.
const BIN: &str = "almanac";
...
fn every_npm_publish_path_is_relative() {
    ...
    // reads it as a folder. `npm publish npm/lmnc-darwin-arm64` cloned
    // `github.com/npm/lmnc-darwin-arm64` and failed on a key it has no
