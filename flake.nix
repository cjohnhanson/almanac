    ...
    in
    {
      packages = forEachSystem (
        ...
        let
          ...
          commonArgs = {
            # pname stays a literal, because the crate is `lmnc` and the
            # command is `almanac`. The version is read from the manifest
            # rather than repeated: pyproject.toml declares it dynamic
            # and npm/generate.mjs greps it, so this was the one place a
          ...
          tool = craneLib.buildPackage (
            ...
            // {
              ...
              postInstall = ''
                ...
                $out/bin/almanac gen-man $out/share/man/man1
                mkdir -p $out/share/zsh/site-functions
                mkdir -p $out/share/bash-completion/completions
                mkdir -p $out/share/fish/vendor_completions.d
                $out/bin/almanac gen-completions zsh > $out/share/zsh/site-functions/_almanac
                $out/bin/almanac gen-completions bash > $out/share/bash-completion/completions/almanac
                $out/bin/almanac gen-completions fish > $out/share/fish/vendor_completions.d/almanac.fish
              '';
